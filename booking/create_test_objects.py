...
sql_formula_hot = "INSERT INTO booking_hotel (name, country, city, slug) VALUES (%s, %s, %s, %s)"
...
def create_test_properties(n):  # creates apartments and hotels
    ...
    for i in range(n):
        ...
        if property_type == "hotel":
            val = (name, country, city, slug)
            my_cursor.execute(sql_formula_hot, val)
            slug = f"{name.lower()}-{str(num)} "
            num +=1
        else:
...
def create_rooms(n):
    ...
    for i in my_result:
        for _ in range(n):
            ...
            if not (room_type, number) in unique:
                my_cursor.execute(sql_formula_rm, (room_type, number))
                unique.append((room_type, number))
    mydb.commit()
    my_cursor.execute("SELECT * FROM booking_room")
    my_result = my_cursor.fetchall()
    for i in my_result:
        print(i)
